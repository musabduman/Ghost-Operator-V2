...
# 1. YÖNETİCİ BEYİN (GEMMA 12B) - Patronla konuşan ve UI'ı yöneten kısım
...
# 2. İŞÇİ BEYİN (QWEN 480B) - Sadece kod yazar, UI ile konuşmaz
class QwenWorker:
    def __init__(self, model="qwen3-coder:480b-cloud"):
        ...
        self.isci_kurallari = """
        Sen sadece bir kod dönüştürme ve yazma motorusun. Asistan değilsin. 
        Kullanıcı sana bir talimat ve (varsa) mevcut bir kod verecek.
        Senden istenen TEK ŞEY, bu talimata uygun olarak güncellenmiş/yazılmış KODU vermektir.
        - Asla 'Merhaba', 'İşte kodun', 'Açıklama:' gibi metinler YAZMA.
        - Asla Markdown kod blokları (```python ... ```) KULLANMA. 
        - SADECE VE SADECE ÇALIŞABİLİR SAF KODU ÇIKTI OLARAK VER.
        """
    ...
    def saf_kod_uret(self, talimat, mevcut_kod=""):
        ...
        payload = {
            ...
            "stream": False,
            "options": {
                "temperature": 0.1 # Sıfıra yakın, halüsinasyonu engeller
            }
        }
        try:
            response = requests.post(self.api_url, json=payload)
...
class GhostController():
    ...
    def yol_duzelt(self, yol):
        ...
        yol = os.path.normpath(yol.replace("/", "\\")) # Slashları düzelt
        
        if "Users\\" in yol:
            parcalar = yol.split("\\")
            if len(parcalar) > 3:
                # 'Musab' kısmını atıp gerçek USER_HOME ile birleştirir
                return os.path.join(user_home, *parcalar[3:])
        return yol
    
    def generate(self, user_input):
        # 1. Her zaman önce Gemma'ya soruyoruz
        cevap = self.supervisor.generate(user_input)
        aktif_model = "GPT-OSS 20B (Yönetici)"
        
        # 2. Gemma "KOD_ISTE" dediyse havada yakalıyoruz!
